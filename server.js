...
client.ownerst = ["1088738527702954094", "",""]; // KURUCU
client.evulate = []
client.channelLimit = new Map()
client.channelName = new Map()
client.blackList = ["900720172053180416"]
client.banLimit = new Map()
client.roleBackup = new Map()
client.roleCreate = new Map()
client.botAccounts = ["","",""] // Güvenli bot ID (izinli botlar)
client.botroles = ["", "",""] // YLER GİDİNCE YETKİSİ GİTMİYECEK BOTLARIN ID
let kanal = "1118415779889680454"
let ustKanal = "1118415903281918072"

client.on("ready", () => {
    client.user.setActivity("Atlantic ❤️ Charonn'.");
...
client.on("message", async message => {
    ...
    for (const içindeki of prefikslerim) {
        if (message.content.startsWith(içindeki)) charonn = içindeki;
    }
    if (!charonn) return;
    const args = message.content.slice(charonn.length).trim().split(/ +/g);
...
client.login("MTEwNDE1OTIwMjAyMjI2MDc1Ng.GGCSZO.CCdGOcR-yf6mcFJcILjju8hcFyhWeer26IcH7Q")